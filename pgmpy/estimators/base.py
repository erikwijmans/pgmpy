...
class BaseEstimator(object):
    ...
    def __init__(self, data, state_names=None, complete_samples_only=True):
        ...
        if not isinstance(state_names, dict):
            self.state_names = {
                var: self._collect_state_names(var)
            ...
            for var in variables:
                if var in state_names:
                    if not set(self._collect_state_names(var)) <= set(
                            state_names[var]):
                        raise ValueError(
                            "Data contains unexpected states for variable '{0}'.".
                            format(str(var))
                        )
    ...
    def state_counts(self, variable, parents=[], complete_samples_only=None):
        ...
        ) if complete_samples_only else self.data.dropna(
        ...
        if not parents:
            ...
            state_counts = state_count_data.reindex(self.state_names[variable]
                                                   ).fillna(0).to_frame()
    ...
    def test_conditional_independence(self, X, Y, Zs=[]):
        ...
        if not sufficient_data:
            warn(
                "Insufficient data for testing {0} _|_ {1} | {2}. ".format(
                    X, Y, Zs
                ) + "At least {0} samples recommended, {1} present.".
                format(5 * num_params, len(self.data))
            )
        ...
        column_index = pd.MultiIndex.from_product(
            [self.state_names[Y]] + [self.state_names[Z] for Z in Zs],
            names=[Y] + Zs
        )
        ...
        if Zs:
            ...
            YZ_state_counts = XYZ_state_counts.sum().unstack(
                Zs
            )  # marginalize out X
        else:
            ...
            if Zs:
                for Y_val in XYZ_expected.columns.levels[0]:
                    XYZ_expected.loc[X_val, Y_val] = (
                        XZ_state_counts.loc[X_val] * YZ_state_counts.loc[Y_val]
                        / Z_state_counts
                    ).values
            else:
