...
class VariableElimination(Inference):
    ...
    def _variable_elimination(
        ...
        elif any(var in elimination_order
                 for var in set(variables).union(set(evidence.keys()
                                                     if evidence else []))):
            raise ValueError(
        ...
        return phi.normalize(inplace=False)
    ...
    def max_marginal(
            self, variables=None, evidence=None, elimination_order=None
    ):
        """
...
class BeliefPropagation(Inference):
    ...
    def _update_beliefs(self, sending_clique, recieving_clique, operation):
        ...
        neighboring ones.
        """
        sepset = frozenset(sending_clique).intersection(
            frozenset(recieving_clique)
        )
    ...
    def _is_converged(self, operation):
        ...
        for edge in self.junction_tree.edges():
            ...
            if (edge[0] not in self.clique_beliefs or
                    edge[1] not in self.clique_beliefs or
            ...
            if marginal_1 != marginal_2 or marginal_1 != self.sepset_beliefs[
                    sepset_key
            ]:
                return False
        return True
            ...
            if not self._is_converged(operation=operation):
                ...
                for neighbor_clique in neighbors:
                    self._update_beliefs(
                        neighbor_clique, clique, operation=operation
                    )
    ...
    def _query(self, variables, operation, evidence=None):
        ...
        for i in range(len(nodes_with_query_variables) - 1):
            subtree_nodes.update(
                nx.shortest_path(
                    self.junction_tree, nodes_with_query_variables[i],
        ...
        if len(subtree.nodes()) == 1:
            ...
            root_node = tuple(
                filter(
                    lambda x: len(list(subtree.neighbors(x))) == 1, subtree.nodes()
                )
            )[0]
        ...
        while parent_nodes:
            ...
            for child_node in set(subtree.neighbors(parent_node)
                                 ) - nodes_traversed:
